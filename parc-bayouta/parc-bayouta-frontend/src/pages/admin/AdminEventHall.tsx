...
import { Calendar, Search, Plus, Eye, Check, X, Ban, Users, PartyPopper } from "lucide-react";
...
export default function AdminEventHall() {
  ...
  // Filter reservations
  const filteredReservations = hallReservations.filter(res => {
    const matchesSearch = 
  ...
  const getReservationForDate = (date: Date) => {
    ...
    return hallReservations.find(res => res.date === dateStr);
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {hallReservations.filter(r => r.status === 'confirmed').reduce((acc, r) => acc + r.guestCount, 0)}
              </div>
            </CardContent>
          </Card>
        </div>
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {calendarDays.map(day => {
                ...
                return (
                  <div
                    key={day.toISOString()}
                    className={`aspect-square p-1 border border-border rounded-lg cursor-pointer transition-colors ${
